...
class GroupAdapter(
    ...
    fun replaceAll(newGroups: List<Group>, newListener: (Long) -> Unit) {
        groups.clear()
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(group: Group, listener: (Long) -> Unit) = with(itemView) {
            group_name_text.text = group.name
            group_creator_text.text = group.creator.name
            group_card.setOnClickListener { listener(group.id) }
        }
